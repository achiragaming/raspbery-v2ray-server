...
log(`Merged ${profileFiles.length} profile(s) — ${(merged.proxies || []).length} proxies found`);
...
function main(config) {
  ...
  const EXTRA_FAKE_IP_BYPASS = [
    '*.lan', '*.local', '*.internal', '*.home',
    'pi.hole', 'host.docker.internal',
  ...
  const vpnServers = [...new Set(
    (config.proxies || [])
      .filter(p => ['vless','vmess','trojan','hysteria2','shadowsocks','tuic'].includes(p.type))
      .map(p => p.server)
  )];

  log(`Auto-detected ${vpnServers.length} VPN server(s): ${vpnServers.join(', ')}`);
...
if (config['proxy-groups']) {
    config['proxy-groups'].forEach(group => {
      ...
      if (group.name.includes('Proxy') || group.name.includes('🚀')) {
        ...
        let otherNodes = group.proxies.filter(p => p !== VPS_PROFILE_NAME && p !== 'DIRECT');
  ...
  const existingRules = (config.rules || []).filter(r => !r.startsWith('MATCH,'));
...
fs.writeFileSync(OUTPUT, yaml.dump(result, {
